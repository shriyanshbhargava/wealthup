...
const NavBar: React.FC<NavbarProps> = ({logged=false, headerSolid: propHeaderSolid, backgroundColor = 'transparent', style}) => {
  const [headerSolid, setHeaderSolid] = useState(propHeaderSolid);
  ...
  interface NavbarProps {
  headerSolid: boolean;
  backgroundColor: string; // Define a backgroundColor prop
}
  ...
  return (
    <header className='relative' style={style}>
      <div className='navigation w-full fixed top-0 left-0 z-30 transition-all duration-300' style={{backgroundColor: headerSolid ? '#035782' : backgroundColor}}>
        <div className='container'>
          <div className='flex justify-between items-center'>
            <div className='px-0 pt-2 md:py-4 relative bottom-2'>
              <Link href={'/'} >
                  <Image
            ...
            <nav className='hidden lg:flex items-center justify-evenly w-10/12 '>
              <div className='px-4'>
                <ul className='flex '>
                  {/* {pages.map(pageitem => {
                  ...
                  <li className="nav-item">
                        <Link href="/about" className="nav-scroll">
                          About Us
                        </Link>
                      </li>
                      <li className="nav-item">
                        <Link href="/#help" className="nav-scroll">
                          Services
                        </Link>
                      </li>
                      ...
                      {/* <li className="nav-item ">
                        <div>
                          <div className="nav-scroll cursor-pointer" onClick={() => setIsOpen(true)}>Join</div>
                        </div>
                      </li> */}
                </ul>
              </div>
        ...
        <>
          <Popover.Button
            ...
            className={`
              ...
              } ${navbar ? ' w-12 h-12 text-xl' : 'bg-primary-blue w-8 h-8 sm:w-12 sm:h-12 text-base md:text-xl'}`}
          >
            <span>{title}</span>
          </Popover.Button>
          <Transition
