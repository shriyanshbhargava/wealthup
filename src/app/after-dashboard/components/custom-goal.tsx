...
export const CustomGoal: React.FC<{
        ...
        <div className="md:w-full w-[90%]">
          {showTitleInput ? (
            <div className="z-50 flex justify-center relative">
              <input autoFocus value={name} onChange={(e) => setName(e.target.value)} type="text" className="bg-white rounded-full p-4 text-2xl text-brand-blue font-bold  w-[80%]" defaultValue="Custom" onKeyDown={handleKeyDown} />
    ...
    <div className="mt-2 flex flex-col">
        <form className="md:w-full w-[90%]" onSubmit={handleSubmit}>
          ...
          <div className="flex flex-col mb-4 mt-6 gap-2">
            <label htmlFor="amount" className="">Amount Needed</label>
            <input type="text" name="amount" defaultValue="₹1,00,000" className="bg-white rounded-full p-4 text-2xl text-brand-blue font-bold" onChange={(e) => {
              const value = e.target.value.replace(/[₹\,]/g, '');
              const numericValue = parseInt(value);
              const formattedValue = isNaN(numericValue) ? '₹0' : `₹${numericValue.toLocaleString('en-IN')}`;
